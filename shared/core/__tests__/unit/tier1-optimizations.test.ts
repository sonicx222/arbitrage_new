...
// ===========================================================================
// T1.5: Chain Staleness Threshold Tests
// ===========================================================================

// Mock the logger before importing WebSocketManager
...
jest.mock('../../src/provider-health-scorer', () => ({
  ...
  ProviderHealthScorer: jest.fn()
}));

describe('T1.5: Chain Staleness Thresholds', () => {
  // We can't easily test the internal threshold without making it accessible
  // But we can verify the config is accepted

  describe('Chain-Specific Defaults', () => {
    it('should configure different thresholds for different chains', () => {
      // This test validates the concept - actual WebSocketManager testing
      ...
      // Fast chains should have 5s threshold
      expect(CHAIN_STALENESS_THRESHOLDS.arbitrum).toBe(5000);
      expect(CHAIN_STALENESS_THRESHOLDS.solana).toBe(5000);

      // Medium chains should have 10s threshold
      expect(CHAIN_STALENESS_THRESHOLDS.polygon).toBe(10000);
      expect(CHAIN_STALENESS_THRESHOLDS.bsc).toBe(10000);
      expect(CHAIN_STALENESS_THRESHOLDS.optimism).toBe(10000);
      expect(CHAIN_STALENESS_THRESHOLDS.base).toBe(10000);

      // Slow chains should have 15s threshold
      expect(CHAIN_STALENESS_THRESHOLDS.ethereum).toBe(15000);
      expect(CHAIN_STALENESS_THRESHOLDS.zksync).toBe(15000);
    });

    it('should have reduced thresholds compared to previous 30s default', () => {
      // All chain thresholds should be less than the old 30s default
      const thresholds = [5000, 10000, 15000];
      const oldDefault = 30000;

      thresholds.forEach(threshold => {
        expect(threshold).toBeLessThan(oldDefault);
      });
    });
  });
});

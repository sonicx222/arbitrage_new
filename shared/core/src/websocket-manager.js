...
const provider_health_scorer_1 = require("./provider-health-scorer");
/**
 ...
 */
function getChainStalenessThreshold(chainId) {
...
class WebSocketManager {
    constructor(config) {
         ...
         */
        this.excludedProviders = new Map();
        /**
        ...
        this.healthScorer = (0, provider_health_scorer_1.getProviderHealthScorer)();
     ...
     */
    selectBestFallbackUrl() {
        const currentUrl = this.getCurrentUrl();
        // Get available (non-excluded) candidates excluding current
        const candidates = this.allUrls.filter(url => url !== currentUrl && !this.isProviderExcluded(url));
     ...
     */
    setIntelligentFallback(enabled) {
    ...
    on(event, handler) {
        ...
        if (event === 'connected' || event === 'disconnected') {
            const wrappedHandler = (connected) => {
     ...
     */
    async resubscribeWithValidation(timeoutMs = 5000) {
     ...
     */
    sendSubscriptionWithTimeout(subscription, timeoutMs) {
     ...
     */
    checkForDataGap(newBlockNumber) {
        const lastKnownBlock = this.qualityMetrics.lastBlockNumber;
        if (lastKnownBlock === 0) {
    ...
    isConnectionStale() {
        ...
        if (this.qualityMetrics.lastMessageTime === 0)
            return false;
     ...
     */
    startProactiveHealthCheck(intervalMs = 10000) {

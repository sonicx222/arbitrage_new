...
/**
 * Internal result from factory query including optional fee tier
 */
...
export class PairDiscoveryService extends EventEmitter {
  ...
  // Statistics
  private stats: PairDiscoveryStats = {
    totalQueries: 0,
    cacheHits: 0,
    factoryQueries: 0,
   ...
   */
  private getFactoryContract(chain: string, dex: Dex): ethers.Contract | null {
    ...
    const contract = new ethers.Contract(dex.factoryAddress, abi, provider);
  ...
  async discoverPair(
    ...
    try {
      ...
      if (queryResult && queryResult.address && queryResult.address !== ethers.ZeroAddress) {
   ...
   */
  private async queryFactory(
    chain: string,
    dex: Dex,
    token0: Token,
    token1: Token
  ): Promise<PoolQueryResult | null> {
   ...
   */
  private async queryFactoryOnce(
    ...
    if (factoryType === 'v3') {
      ...
      const poolPromises = V3_FEE_TIERS.map(async feeTier => {
        try {
          const result = await this.withTimeout(
            contract.getPool(token0.address, token1.address, feeTier),
            timeoutMs
          );
      ...
      for (const result of results) {
        if (result && result.address && result.address !== ethers.ZeroAddress) {
          return result;
        }
      }
      return null;
      ...
      const pairAddress = await this.withTimeout(
        contract.getPair(token0.address, token1.address),
        timeoutMs
      );
   ...
   */
  private createTimeoutWithCleanup(ms: number): [Promise<never>, () => void] {
  ...
  computePairAddress(
    ...
    const packed = ethers.solidityPacked(
      ...
      ['0xff', dex.factoryAddress, salt, initCodeHash]
    );

    const address = '0x' + ethers.keccak256(packed).slice(26);
   ...
   */
  async discoverPairsBatch(
    chain: string,
    dex: Dex,
    tokenPairs: Array<{ token0: Token; token1: Token }>
    ...
    const semaphoreSlots: Promise<void>[] = new Array(this.config.maxConcurrentQueries)
      .fill(null)
      .map(() => Promise.resolve());
    let slotIndex = 0;

    const processWithConcurrency = async (token0: Token, token1: Token): Promise<DiscoveredPair | null> => {
    ...
    for (let i = 0; i < tokenPairs.length; i += this.config.batchSize) {
      ...
      const batchPromises = batch.map(({ token0, token1 }) =>
        processWithConcurrency(token0, token1)
      );
  ...
  detectFactoryType(dexName: string): 'v2' | 'v3' | 'curve' | 'unsupported' {
    ...
    if (nameLower.includes('v3') || nameLower.includes('_v3')) return 'v3';
    // S3.2.1-FIX: KyberSwap Elastic uses concentrated liquidity (V3-style getPool)
    if (nameLower.includes('kyberswap') || nameLower.includes('kyber')) return 'v3';

    // Curve-style DEXs (multi-asset stable pools)
    if (nameLower.includes('curve') || nameLower.includes('ellipsis')) return 'curve';
    ...
    if (nameLower.includes('gmx') || nameLower.includes('platypus')) return 'unsupported';
   ...
   */
  private getInitCodeHash(dexName: string): string | null {
  ...
  private isCircuitOpen(chain: string, dexName: string): boolean {
  ...
  private resetFailureCount(chain: string, dexName: string): void {
    const key = `${chain}:${dexName}`;
    this.failureCount.delete(key);
  }
...
let pairDiscoveryInstance: PairDiscoveryService | null = null;
...
export function resetPairDiscoveryService(): void {

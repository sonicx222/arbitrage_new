...
exports.SharedMemoryCache = void 0;
exports.createSharedMemoryCache = createSharedMemoryCache;
exports.getSharedMemoryCache = getSharedMemoryCache;
const logger_1 = require("./logger");
const logger = (0, logger_1.createLogger)('shared-memory-cache');
...
class SharedMemoryCache {
    ...
    keys() {
        ...
        try {
            ...
            for (let i = 0; i < entryCount; i++) {
                if (offset + SharedMemoryCache.ENTRY_HEADER_SIZE >= this.dataView.length)
                    break;
    ...
    updateEntry(key, value, ttl, existingEntry) {
    ...
    simpleEncrypt(data) {
        ...
        return data.split('').map(char => String.fromCharCode(char.charCodeAt(0) ^ 0x55)).join('');
    }
    simpleDecrypt(data) {
...
exports.SharedMemoryCache = SharedMemoryCache;
// Constants for memory layout
SharedMemoryCache.METADATA_SIZE = 16; // 4 * 4 bytes
SharedMemoryCache.ENTRY_HEADER_SIZE = 24; // keyLen(4) + valueLen(4) + timestamp(8) + flags(4) + ttl(4)
SharedMemoryCache.MAX_KEY_LENGTH = 256;
SharedMemoryCache.MAX_VALUE_LENGTH = 1024 * 1024; // 1MB per value
// Metadata offsets (in 32-bit words)
SharedMemoryCache.VERSION_OFFSET = 0;
SharedMemoryCache.ENTRY_COUNT_OFFSET = 1;
SharedMemoryCache.MAX_ENTRIES_OFFSET = 2;
SharedMemoryCache.DATA_START_OFFSET = 3;
// Factory function
function createSharedMemoryCache(config) {
    return new SharedMemoryCache(config);
}
// Default instance
let defaultSharedCache = null;
function getSharedMemoryCache() {
...
//# sourceMappingURL=shared-memory-cache.js.map
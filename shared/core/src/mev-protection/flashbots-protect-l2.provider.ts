...
export class FlashbotsProtectL2Provider extends BaseMevProvider {
  ...
  async healthCheck(): Promise<{ healthy: boolean; message: string }> {
    ...
    try {
      ...
      return {
        healthy: false,
        message: `Failed to reach Base sequencer: ${error instanceof Error ? error.message : String(error)}`,
      };
    }
  }
  ...
  private async checkProtectHealth(): Promise<{ healthy: boolean; message: string }> {
    ...
    try {
      ...
      return {
        healthy: false,
        message: `Failed to reach Protect endpoint: ${error instanceof Error ? error.message : String(error)}`,
      };
    } finally {
      clearTimeout(timeoutId);
    }

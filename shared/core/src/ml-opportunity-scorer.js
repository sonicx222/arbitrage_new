...
exports.MLOpportunityScorer = void 0;
exports.getMLOpportunityScorer = getMLOpportunityScorer;
exports.resetMLOpportunityScorer = resetMLOpportunityScorer;
const logger_1 = require("./logger");
const logger = (0, logger_1.createLogger)('ml-opportunity-scorer');
// =============================================================================
...
class MLOpportunityScorer {
     ...
     */
    async enhanceOpportunityScore(input) {
        ...
        const directionAligned = this.checkDirectionAlignment(mlPrediction.direction, opportunityDirection);
        // Calculate price impact score (magnitude of predicted move)
        const priceImpactScore = this.calculatePriceImpactScore(currentPrice, mlPrediction.predictedPrice);
     ...
     */
    async enhanceWithMomentum(input) {
        ...
        const momentumAligned = this.checkMomentumAlignment(momentumSignal.trend, input.opportunityDirection);
        ...
        const finalConfidence = Math.max(0, Math.min(1, mlResult.enhancedConfidence * adjustedWeight + momentumContribution));
        return {
            ...mlResult,
     ...
     */
    async rankOpportunities(inputs) {
    ...
    isValidPrediction(prediction) {
        if (!prediction)
            return false;
        if (typeof prediction.predictedPrice !== 'number' || isNaN(prediction.predictedPrice))
            return false;
        if (typeof prediction.confidence !== 'number' || prediction.confidence < 0)
            return false;
        if (!['up', 'down', 'sideways'].includes(prediction.direction))
            return false;
        return true;
    }
    /**
     ...
     */
    checkDirectionAlignment(mlDirection, oppDirection) {
        if (mlDirection === 'sideways')
            return true; // Neutral alignment
        if (oppDirection === 'buy')
            return mlDirection === 'up';
        if (oppDirection === 'sell')
     ...
     */
    checkMomentumAlignment(trend, oppDirection) {
        if (trend === 'neutral')
            return true;
        if (oppDirection === 'buy')
     ...
     */
    calculatePriceImpactScore(currentPrice, predictedPrice) {
...
exports.MLOpportunityScorer = MLOpportunityScorer;
// =============================================================================
// Singleton Factory
// =============================================================================
/**
 ...
 */
function getMLOpportunityScorer(config) {
    if (!scorerInstance) {
 ...
 */
function resetMLOpportunityScorer() {
    if (scorerInstance) {
        scorerInstance.resetStats();
    }
    scorerInstance = null;
}
//# sourceMappingURL=ml-opportunity-scorer.js.map
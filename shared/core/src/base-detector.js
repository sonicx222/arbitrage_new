...
class BaseDetector {
    constructor(config, deps) {
        ...
        this.logger = deps?.logger ?? (0, index_1.createLogger)(`${this.chain}-detector`);
        this.perfLogger = deps?.perfLogger ?? (0, index_1.getPerformanceLogger)(`${this.chain}-detector`);
    ...
    async checkIntraDexArbitrage(pair) {
        ...
        for (const otherPair of matchingPairs) {
            ...
            if (otherPair.address.toLowerCase() === currentSnapshot.address.toLowerCase())
                continue;
            if (otherPair.dex === currentSnapshot.dex)
                continue;
    ...
    calculateArbitrageOpportunity(sourceUpdate, targetUpdate) {
        try {
            ...
            const gasCache = (0, index_1.getGasPriceCache)();
            const gasCostEstimate = gasCache.estimateGasCostUsd(this.chain, index_1.GAS_UNITS.simpleSwap);
            const dynamicGasCost = gasCostEstimate.costUsd || src_1.ARBITRAGE_CONFIG.estimatedGasCost;
            const opportunity = {
                ...
                netProfit: (src_1.ARBITRAGE_CONFIG.defaultAmount * netPercentage) - dynamicGasCost,
                confidence,
                timestamp: Date.now(),
     ...
     */
    getPairsForTokens(token0, token1) {
     ...
     */
    updatePairInTokenIndex(oldPair, newPair) {

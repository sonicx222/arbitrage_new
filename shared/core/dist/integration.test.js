...
class MockRedisClient {
    ...
    async xreadgroup(groupOption, group, consumerOption, consumer, ...args) {
        this.commandCount++;
        // Return mock messages
        return [];
    }
    async xgroup(command, stream, group, ...args) {
        this.commandCount++;
...
describe('DistributedLockManager Integration', () => {
    ...
    beforeEach(async () => {
        ...
        await lockManager.initialize(mockRedis);
    });
...
describe('ServiceStateManager Integration', () => {
    ...
    describe('State Transitions', () => {
        it('should start in STOPPED state', () => {
            expect(stateManager.getState()).toBe(service_state_1.ServiceState.STOPPED);
        });
        // Skip: State transitions require proper mock setup
        it.skip('should transition through valid lifecycle', async () => {
            // STOPPED -> STARTING
            expect(await stateManager.transitionTo(service_state_1.ServiceState.STARTING)).toBe(true);
            expect(stateManager.getState()).toBe(service_state_1.ServiceState.STARTING);
            // STARTING -> RUNNING
            expect(await stateManager.transitionTo(service_state_1.ServiceState.RUNNING)).toBe(true);
            expect(stateManager.getState()).toBe(service_state_1.ServiceState.RUNNING);
            // RUNNING -> STOPPING
            expect(await stateManager.transitionTo(service_state_1.ServiceState.STOPPING)).toBe(true);
            expect(stateManager.getState()).toBe(service_state_1.ServiceState.STOPPING);
            // STOPPING -> STOPPED
            expect(await stateManager.transitionTo(service_state_1.ServiceState.STOPPED)).toBe(true);
            expect(stateManager.getState()).toBe(service_state_1.ServiceState.STOPPED);
        });
        // Skip: State transitions require proper mock setup
        it.skip('should reject invalid transitions', async () => {
            // Cannot go from STOPPED to RUNNING directly
            expect(await stateManager.transitionTo(service_state_1.ServiceState.RUNNING)).toBe(false);
            expect(stateManager.getState()).toBe(service_state_1.ServiceState.STOPPED);
        });
        // Skip: State transitions require proper mock setup
        it.skip('should handle error state transitions', async () => {
            ...
            expect(await stateManager.transitionTo(service_state_1.ServiceState.ERROR)).toBe(true);
            expect(stateManager.getState()).toBe(service_state_1.ServiceState.ERROR);
    ...
    describe('State Guards', () => {
        ...
        it('should allow start from stopped state', () => {
            ...
            stateManager.transitionTo(service_state_1.ServiceState.STARTING);
    ...
    describe('Concurrent State Changes', () => {
        ...
        it.skip('should handle concurrent transition attempts safely', async () => {
            ...
            const results = await Promise.all([
                stateManager.transitionTo(service_state_1.ServiceState.STOPPING),
                stateManager.transitionTo(service_state_1.ServiceState.STOPPING),
                stateManager.transitionTo(service_state_1.ServiceState.STOPPING)
            ]);
...
describe('Cross-Component Integration', () => {
        ...
        it.skip('should coordinate service startup with lock', async () => {
            const startService = async () => {
                ...
                try {
                    ...
                    if (!await stateManager.transitionTo(service_state_1.ServiceState.STARTING)) {
                        return { started: false, reason: 'transition_failed' };
                    }
                    // Simulate initialization
                    await new Promise(resolve => setTimeout(resolve, 10));
                    if (!await stateManager.transitionTo(service_state_1.ServiceState.RUNNING)) {
    ...
    describe('Execution Coordination', () => {
        it('should prevent duplicate opportunity execution', async () => {
            const executionLog = [];
            const executeOpportunity = async (oppId) => {
...
describe('Performance Integration', () => {
    it('should handle high message throughput', async () => {
        ...
        for (let i = 0; i < messageCount; i++) {
            await mockRedis.xadd('test:stream', '*', 'type', 'price_update', 'data', JSON.stringify({ price: Math.random() }));
        }
    ...
    it('should maintain lock performance under load', async () => {
        const mockRedis = new MockRedisClient();
        const lockManager = new distributed_lock_1.DistributedLockManager();
        await lockManager.initialize(mockRedis);

...
exports.enqueueFailedOperation = enqueueFailedOperation;
const logger_1 = require("./logger");
const redis_1 = require("./redis");
const redis_streams_1 = require("./redis-streams");
const logger = (0, logger_1.createLogger)('dead-letter-queue');
class DeadLetterQueue {
        ...
        try {
            ...
            for (const key of keys) {
                if (key.startsWith('dlq:') && !key.includes(':priority:') && !key.includes(':service:') && !key.includes(':tag:')) {
                    ...
                    const operation = await redis.get(key);
    ...
    startAutoProcessing(intervalMs = 30000) {
    ...
    async simulateOperationProcessing(operation) {
        ...
        const rate = failureRate[operation.operation] || 0.1;
        if (Math.random() < rate) {
    ...
    async scheduleRetry(operationId) {
    ...
    async findOperationInIndexes(operationId) {
        const redis = await this.redis;
        const indexKeys = [];
    ...
    async checkAlertThreshold() {
        ...
        if (size > this.config.alertThreshold) {
            ...
            await this.dualPublish('stream:dlq-alerts', // Primary: Redis Streams
...
// Global DLQ instance
let globalDLQ = null;
function getDeadLetterQueue(config) {
    if (!globalDLQ) {
...
async function enqueueFailedOperation(operation) {

...
class TestDetector extends base_detector_1.BaseDetector {
    constructor(config) {
        super({
            chain: 'ethereum',
            enabled: true,
        ...
        this.testCalls = [];
        this.mockPriceUpdates = [];
        this.mockArbitrageOpportunities = [];
        this.mockWhaleTransactions = [];
    }
    // Expose protected methods for testing
    async testProcessSyncEvent(log, pair) {
        return this.processSyncEvent(log, pair);
    }
    async testProcessSwapEvent(log, pair) {
        return this.processSwapEvent(log, pair);
    }
    async testCheckIntraDexArbitrage(pair) {
        return this.checkIntraDexArbitrage(pair);
    }
    async testCheckWhaleActivity(swapEvent) {
        return this.checkWhaleActivity(swapEvent);
    }
    async testEstimateUsdValue(pair, amount0In, amount1In, amount0Out, amount1Out) {
        return this.estimateUsdValue(pair, amount0In, amount1In, amount0Out, amount1Out);
    }
    testCalculatePrice(pair) {
        return this.calculatePrice(pair);
    }
    testCreatePairSnapshot(pair) {
        return this.createPairSnapshot(pair);
    }
    testCreatePairsSnapshot() {
        return this.createPairsSnapshot();
    }
    testCalculatePriceFromSnapshot(snapshot) {
    ...
    async publishWhaleTransaction(transaction) {
...
// Test Fixtures
// =============================================================================
function createMockPair(overrides) {
    return {
        name: 'WETH/USDC',
...
function createMockSyncLog(pair, reserve0, reserve1) {
...
function createMockSwapLog(pair) {
    return {
        address: pair.address,
        topics: [
    ...
    describe('USD Value Estimation', () => {
        describe('estimateUsdValue()', () => {
            it('should estimate USD value for native token swaps', async () => {
                ...
                const usdValue = await detector.testEstimateUsdValue(pair, '1000000000000000000', // 1 ETH in
                '0', '0', '2500000000' // 2500 USDC out
                );
            ...
            it('should return 0 for unknown token pairs', async () => {
                ...
                const usdValue = await detector.testEstimateUsdValue(pair, '1000000000000000000', '0', '0', '1000000000000000000');
...
//# sourceMappingURL=base-detector.test.js.map
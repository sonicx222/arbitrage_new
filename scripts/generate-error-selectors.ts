...
function main(): void {
  console.log('🔧 Generating error selectors from FlashLoanArbitrage ABI...\n');

  // Read ABI file
  if (!fs.existsSync(ABI_PATH)) {
    console.error(`❌ ABI file not found: ${ABI_PATH}`);
  ...
  if (errors.length === 0) {
    console.warn('⚠️  No custom errors found in ABI');
    process.exit(0);
  }
 ...
 * Source: ${path.relative(path.join(__dirname, '..'), ABI_PATH)}
 *

...
describe('P2 L2-Turbo Partition Service Integration', () => {
  ...
  describe('Detector Configuration', () => {
    ...
    it('should configure detector with all 3 L2 chains by default', async () => {
      jest.resetModules();
      const { detector, cleanupProcessHandlers } = await import('../../index');
      cleanupFn = cleanupProcessHandlers;
  ...
  describe('Service Configuration', () => {
    it('should use custom INSTANCE_ID from environment', async () => {
      process.env.INSTANCE_ID = 'custom-l2-turbo-123';

      jest.resetModules();
      const { config, cleanupProcessHandlers } = await import('../../index');
      cleanupFn = cleanupProcessHandlers;

      expect(config.instanceId).toBe('custom-l2-turbo-123');
    });
  ...
  describe('Process Handler Setup', () => {
    it('should call setupProcessHandlers with correct parameters', async () => {
      ...
      expect(callArgs[3]).toBe('partition-l2-turbo'); // serviceName
    });
  ...
  describe('Uses Centralized Constants', () => {
  ...
  describe('L2 Chain Characteristics', () => {
    it('should have Arbitrum in the partition', async () => {
      jest.resetModules();
      const { config, cleanupProcessHandlers } = await import('../../index');
      cleanupFn = cleanupProcessHandlers;
    ...
    it('should have Optimism in the partition', async () => {
      jest.resetModules();
      const { config, cleanupProcessHandlers } = await import('../../index');
      cleanupFn = cleanupProcessHandlers;
    ...
    it('should have Base in the partition', async () => {
      jest.resetModules();
      const { config, cleanupProcessHandlers } = await import('../../index');
      cleanupFn = cleanupProcessHandlers;

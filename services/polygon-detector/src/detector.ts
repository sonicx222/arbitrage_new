 ...
 *
 * Monitors Polygon chain DEXes for arbitrage opportunities.
 ...
 *
 * Optimized for Polygon's 2-second block time
 *
...
interface PolygonPair extends Pair {
  reserve0: string;
  reserve1: string;
  blockNumber: number;
  lastUpdate: number;
...
interface PolygonDetectorConfig {
  chain: string;
  enabled: boolean;
...
// =============================================================================
// Polygon Detector Service
// =============================================================================

export class PolygonDetectorService extends BaseDetector {
  private readonly polygonConfig: PolygonDetectorConfig;
  ...
  private readonly tokenMetadata = TOKEN_METADATA.polygon;

  constructor() {
    const config: PolygonDetectorConfig = {
      chain: 'polygon',
      enabled: true,
      wsUrl: CHAINS.polygon?.wsUrl,
      rpcUrl: CHAINS.polygon?.rpcUrl || 'https://polygon-rpc.com',
      batchSize: 20,
      batchTimeout: 30,
  ...
  getConfig(): PolygonDetectorConfig {
    return { ...this.polygonConfig };
  }

  getMinProfitThreshold(): number {
    return ARBITRAGE_CONFIG.chainMinProfits.polygon || 0.003; // 0.3%
  }

  getSupportedDexes(): string[] {
    return (DEXES.polygon || []).map(dex => dex.name);
  }

  getDexConfigs(): Dex[] {
    return DEXES.polygon || [];
  }

  getSupportedTokens(): Token[] {
    return CORE_TOKENS.polygon || [];
  }
  ...
  private async performCleanup(): Promise<void> {
    try {
      ...
      this.logger.info('Polygon detector service stopped');
    } finally {
  ...
  protected async initializePairs(): Promise<void> {
    ...
    this.logger.info(`Initialized ${this.pairs.size} trading pairs on Polygon`);
  }
  ...
  protected async processSyncEvent(log: any, pair: Pair): Promise<void> {
    try {
      ...
      // Update pair data
      const polygonPair = pair as PolygonPair;
      polygonPair.reserve0 = reserve0;
      polygonPair.reserve1 = reserve1;
      polygonPair.blockNumber = blockNumber;
      polygonPair.lastUpdate = Date.now();

      // Calculate price
      const price = this.calculatePrice(polygonPair);
  ...
  protected calculatePrice(pair: PolygonPair | { reserve0: string; reserve1: string; token0: string; token1: string }): number {
    try {
  ...
  protected async estimateUsdValue(
    ...
    const maticPrice = 0.80; // TODO: Fetch from price oracle in Phase 2
    const token0Lower = pair.token0.toLowerCase();
    const token1Lower = pair.token1.toLowerCase();
    const wmaticLower = this.tokenMetadata.wmatic.toLowerCase();

    // Check if either token is WMATIC
    if (token0Lower === wmaticLower || token1Lower === wmaticLower) {
      const isToken0Wmatic = token0Lower === wmaticLower;
      const amount = isToken0Wmatic
    ...
    for (const [key, otherSnapshot] of pairsSnapshots) {
      ...
      if (sameOrder || reverseOrder) {
        ...
        if (priceDiff >= this.getMinProfitThreshold()) {
          ...
          const chainConfig = DETECTOR_CONFIG.polygon;
  ...
  protected async checkWhaleActivity(swapEvent: SwapEvent): Promise<void> {
    ...
    const whaleThreshold = DETECTOR_CONFIG.polygon.whaleThreshold;
  ...
  protected async calculatePriceImpact(swapEvent: SwapEvent): Promise<number> {
    ...
    const pair = this.pairsByAddress.get(swapEvent.pairAddress.toLowerCase()) as PolygonPair;
  ...
  private startHealthMonitoring(): void {
    ...
    }, this.polygonConfig.healthCheckInterval);
  }
}

    ...
    (async (_req: Request, res: Response) => {
      try {
        ...
        const coordinatorMetrics = [
          ...
          // P2-6 FIX: Forwarding rejection counters — previously only in /api/health JSON,
          // not scrapable by Grafana. Each counter tracks a specific rejection reason.
          ...(sys.forwardingMetrics ? [
          ...
          // P2-6 FIX: DLQ error classification counters for alerting on error patterns.
          ...(sys.dlqMetrics ? [

...
export class BaseDetectorService {
  private provider: ethers.JsonRpcProvider;
  private wsProvider: WebSocket | null = null;
  private redis = getRedisClient();
  private logger = createLogger('base-detector');
  ...
  constructor() {
    this.perfLogger = getPerformanceLogger('base-detector');
    this.dexes = DEXES.base;
    this.tokens = CORE_TOKENS.base;
    this.provider = new ethers.JsonRpcProvider(CHAINS.base.rpcUrl);
  }
  ...
  private async checkWhaleActivity(swapEvent: SwapEvent): Promise<void> {
    if (!swapEvent.usdValue || swapEvent.usdValue < 50000) return; // $50K threshold for Base (Coinbase ecosystem)

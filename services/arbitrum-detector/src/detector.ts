...
export class ArbitrumDetectorService {
  private provider: ethers.JsonRpcProvider;
  private wsProvider: WebSocket | null = null;
  private redis = getRedisClient();
  private logger = createLogger('arbitrum-detector');
  ...
  private eventBuffer: any[] = []; // Buffer for ultra-fast processing
  private processingInterval: NodeJS.Timeout | null = null;

  constructor() {
    this.perfLogger = getPerformanceLogger('arbitrum-detector');
    this.dexes = DEXES.arbitrum;
    this.tokens = CORE_TOKENS.arbitrum;
    this.provider = new ethers.JsonRpcProvider(CHAINS.arbitrum.rpcUrl);
  }
  ...
  private async connectWebSocket(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        const wsUrl = CHAINS.arbitrum.wsUrl;
  ...
  private startUltraFastProcessing(): void {
  ...
  private async checkWhaleActivity(swapEvent: SwapEvent): Promise<void> {
    if (!swapEvent.usdValue || swapEvent.usdValue < 25000) return; // $25K threshold for Arbitrum (lower due to gas costs)

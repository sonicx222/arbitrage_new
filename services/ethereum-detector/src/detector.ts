 ...
 *
 * Monitors Ethereum mainnet DEXes for arbitrage opportunities.
 ...
 *
 * Optimized for Ethereum's 12-second block time
 *
 * @see IMPLEMENTATION_PLAN.md S2.6
...
interface EthereumPair extends Pair {
  reserve0: string;
  reserve1: string;
  blockNumber: number;
  lastUpdate: number;
...
interface EthereumDetectorConfig {
  chain: string;
  enabled: boolean;
...
// =============================================================================
// Ethereum Detector Service
// =============================================================================

export class EthereumDetectorService extends BaseDetector {
  private readonly ethereumConfig: EthereumDetectorConfig;
  ...
  private readonly tokenMetadata = TOKEN_METADATA.ethereum;

  constructor() {
    const config: EthereumDetectorConfig = {
      chain: 'ethereum',
      enabled: true,
      wsUrl: CHAINS.ethereum?.wsUrl,
      rpcUrl: CHAINS.ethereum?.rpcUrl || 'https://eth.llamarpc.com',
  ...
  getConfig(): EthereumDetectorConfig {
    return { ...this.ethereumConfig };
  }

  getMinProfitThreshold(): number {
    return ARBITRAGE_CONFIG.chainMinProfits.ethereum || 0.005; // 0.5% (higher due to gas costs)
  }

  getSupportedDexes(): string[] {
    return (DEXES.ethereum || []).map(dex => dex.name);
  }

  getDexConfigs(): Dex[] {
    return DEXES.ethereum || [];
  }

  getSupportedTokens(): Token[] {
    return CORE_TOKENS.ethereum || [];
  }
  ...
  private async performCleanup(): Promise<void> {
    try {
      ...
      this.logger.info('Ethereum detector service stopped');
    } finally {
  ...
  protected async initializePairs(): Promise<void> {
    ...
    this.logger.info(`Initialized ${this.pairs.size} trading pairs on Ethereum`);
  }
  ...
  protected async processSyncEvent(log: any, pair: Pair): Promise<void> {
    try {
      ...
      // Update pair data
      const ethereumPair = pair as EthereumPair;
      ethereumPair.reserve0 = reserve0;
      ethereumPair.reserve1 = reserve1;
      ethereumPair.blockNumber = blockNumber;
      ethereumPair.lastUpdate = Date.now();

      // Calculate price
      const price = this.calculatePrice(ethereumPair);
  ...
  protected calculatePrice(pair: EthereumPair | { reserve0: string; reserve1: string; token0: string; token1: string }): number {
    try {
    ...
    for (const [key, otherSnapshot] of pairsSnapshots) {
      ...
      if (sameOrder || reverseOrder) {
        ...
        if (priceDiff >= this.getMinProfitThreshold()) {
          ...
          const chainConfig = DETECTOR_CONFIG.ethereum;
  ...
  protected async checkWhaleActivity(swapEvent: SwapEvent): Promise<void> {
    ...
    const whaleThreshold = DETECTOR_CONFIG.ethereum.whaleThreshold;
  ...
  protected async calculatePriceImpact(swapEvent: SwapEvent): Promise<number> {
    ...
    const pair = this.pairsByAddress.get(swapEvent.pairAddress.toLowerCase()) as EthereumPair;
  ...
  private startHealthMonitoring(): void {
    ...
    }, this.ethereumConfig.healthCheckInterval);
  }
}

...
// Unified Chain Detector
// =============================================================================
class UnifiedChainDetector extends events_1.EventEmitter {
    logger;
    perfLogger;
    stateManager;
    redis = null;
    ...
    async startChainInstances() {
        ...
        for (const chainId of this.config.chains) {
            ...
            startPromises.push(instance.start().catch((error) => {
    ...
    static CHAIN_STOP_TIMEOUT_MS = 30000; // 30 seconds
    async stopChainInstances() {
        const stopPromises = [];
        ...
        for (const [chainId, instance] of instancesSnapshot) {
            ...
            const stopWithTimeout = Promise.race([
                instance.stop(),
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Chain ${chainId} stop timeout after ${UnifiedChainDetector.CHAIN_STOP_TIMEOUT_MS}ms`)), UnifiedChainDetector.CHAIN_STOP_TIMEOUT_MS))
            ]).catch((error) => {
                this.logger.error(`Error stopping chain instance: ${chainId}`, { error: error.message });
            });
    ...
    handleChainError(chainId, error) {
        ...
        if (this.degradationManager) {
            this.degradationManager.triggerDegradation(`unified-detector-${this.config.partitionId}`, `chain_${chainId}_failure`, error);
        }
    ...
    startMetricsCollection() {
        this.metricsInterval = setInterval(() => {
            ...
            this.perfLogger.logHealthCheck(`unified-detector-${this.config.partitionId}`, {
                status: 'healthy',
...
exports.UnifiedChainDetector = UnifiedChainDetector;
